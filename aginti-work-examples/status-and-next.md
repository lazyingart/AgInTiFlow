...
Before the next broad profile run, keep two control regressions in scope: verify that `/status`, `Esc`, `Ctrl+C`, resume pagination, explicit `--cwd`, explicit package policy, and read-only missing-path failures remain stable in the installed CLI after every interactive-shell change; also verify one real non-text artifact task (PDF or image) keeps its semantic filename, exact artifact root, content validation, and path-free delivery reply.
