...
async function fixture(t, { length = 775166, interruption = "", pause = "", wrongScope = false } = {}) {
  ...
  const runner = {
    async run(scope, input, options) {
      ...
      runScope = scope;
      const issue = { ...issueRequest(bytes), scope: wrongScope ? { ...scope, principalId: "principal.wrong-paper-owner" } : scope };
      ...
      const done = result(imported.artifacts);
  ...
  function open() {
    service = createTestOnlyIntegrationAnalysisSessionService({
      stateRoot, analysisRunner: runner, fileWorkerClient: client, fileWorkerEnabled: true,
    });
  }
  open();
  const thread = (await service.createThread({ title: "Selected source paper" }, context)).thread;
  const run = (await service.startRun({ threadId: thread.id, input: { text: prompt } }, context)).run;
...
async function finished(f) {
  ...
  return (await f.service.listArtifacts({ runId: f.run.id }, context)).artifacts;
}
...
async function events(f) {
  ...
  return response.publicEventLedger.loadEventsAfter(0);
}
...
test("a 16 MiB paper survives conversation storage, restart, scoped content and deletion", async t => {
  ...
  await f.restart();
  assert.deepEqual((await f.service.listArtifacts({ runId: f.run.id }, context)).artifacts, artifacts);
  assert.equal(sha256((await content(f, artifact.id)).bytes), sha256(f.bytes));
  ...
  assert.equal(f.runnerCalls[1].priorArtifacts[0].id, artifact.id);
...
for (const interruption of ["before", "after"]) test(`paper commit recovers after interruption ${interruption} worker commit`, async t => {
  ...
  assert.deepEqual((await f.service.listArtifacts({ runId: f.run.id }, context)).artifacts, []);
  ...
  assert.deepEqual(saved.envelope.state.artifacts[0].paperReceipt.source, source);
  await f.restart();
  f.setAvailable(true);
  const artifacts = await finished(f);
  assert.equal(artifacts.length, 1);
...
for (const pause of ["before", "after"]) test(`cancellation ${pause} paper commit cleans the file through restart/outage`, { timeout: 10_000 }, async t => {
  ...
  assert.equal(recovered.envelope.state.artifacts.length, 0);
...
test("a paper sealed for another account is rejected before conversation capture", async t => {
  ...
  const saved = await f.persisted();
  assert.equal(saved.envelope.state.artifacts.length, 0);
...
for (const mutation of ["source", "receipt-missing", "profile", "bytes", "scope"]) test(`persisted paper ${mutation} tampering fails closed`, async t => {
  const f = await fixture(t);
  await finished(f);
  await f.service.close({ mode: "wait" });
  const saved = await f.persisted();
  const record = saved.envelope.state.artifacts[0];
