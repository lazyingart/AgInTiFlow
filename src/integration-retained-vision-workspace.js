...
function buildAttestation(state) {
  const unsigned = frozenRecord({
    ...
    nativeWriteFenceAttestationDigest: state.nativeWriteFence.attestation.digest,
    nativeWriteFenceSealDigest:
      state.nativeWriteFence.attestation.sessionStateWriteFenceSealDigest,
...
export function assertRetainedIntegrationVisionWorkspace(value, expected = {}) {
  ...
  assertRetainedIntegrationNativeExecutionEvidence(state.evidence, {
    ...
    sessionStateStoreExpected: state.sessionExpected,
  ...
  assertRetainedIntegrationTextWorkspace(state.textWorkspace, {
    sessionStateStore: state.sessionStore,
    sessionStateStoreExpected: state.sessionExpected,
    nativeExecutionEvidence: state.evidence,
    nativeWriteFence: state.nativeWriteFence,
...
async function currentVisionWorkspaceProof(state) {
  ...
  if (
    textProof.nativeWriteFenceAttestationDigest !== nativeWriteFence.attestation.digest ||
    textProof.nativeWriteFenceSealDigest !==
      nativeWriteFence.attestation.sessionStateWriteFenceSealDigest ||
    state.attestation.nativeWriteFenceAttestationDigest !== nativeWriteFence.attestation.digest ||
    state.attestation.nativeWriteFenceSealDigest !==
      nativeWriteFence.attestation.sessionStateWriteFenceSealDigest
  ) {
    fail("INTEGRATION_VISION_WORKSPACE_UNAVAILABLE", "Vision-workspace native-write fence binding changed.");
  }
  ...
  const unsigned = frozenRecord({
    ...
    nativeSessionStateWriterFencing: textProof.nativeSessionStateWriterFencing === true,
    nativeSessionStateWriterQuiescenceProven:
      textProof.nativeSessionStateWriterQuiescenceProven === true,
    fullSessionStoreSidecarsFenced: false,
    imagePerceptionSidecarsFenced: false,
    crossProcessImageWriterFencing: false,
  });
