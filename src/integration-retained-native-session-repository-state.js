...
export function assertRetainedIntegrationNativeSessionRepositoryStateUsesSessionStateStore(
  value,
  expectedInput,
  candidateSessionStateStore
) {
  assertRetainedIntegrationNativeSessionRepositoryState(value, expectedInput);
  const state = repositoryStateBrand.get(value);
  if (
    !state || value !== state.surface ||
    state.sessionStateStore !== candidateSessionStateStore
  ) {
    stateFail(
      "INTEGRATION_REPOSITORY_KERNEL_UNAVAILABLE",
