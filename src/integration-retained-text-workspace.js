...
export const INTEGRATION_RETAINED_TEXT_WORKSPACE_ATTESTATION_VERSION =
  "aginti-retained-text-workspace-attestation-v1";
...
function buildAttestation(store, expected, evidence) {
  const unsigned = frozenRecord({
...
export function createRetainedIntegrationTextWorkspace(input = {}) {
  const options = exactPayload(
    input,
    ["sessionStateStore", "sessionStateStoreExpected", "nativeExecutionEvidence"],
    [],
  ...
  const attestation = buildAttestation(store, expected, evidence);
  const state = { store, evidence, expected, attestation };
