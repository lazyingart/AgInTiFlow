...
const UNSUPPORTED_NATIVE_HOST_TOOL_ACTION =
  /\b(?:use|call|invoke|run|execute)\s+(?:the\s+)?(?!execute_python_analysis\b|grounded_search\b|deep_research\b|compile_tex_document\b|publish_file_artifact\b)(?:(?:tmux|terminal|shell|systemd|journalctl|lsof|netstat|host|native|workstation|ps)[a-z0-9]*(?:_[a-z0-9]+)+|[a-z][a-z0-9]*(?:_[a-z0-9]+)*(?:_tmux|_terminal|_shell|_systemd|_journalctl|_lsof|_netstat|_host|_native|_workstation|_pid|_pids|_processes)(?:_[a-z0-9]+)*)\b/iu;
const UNSUPPORTED_NATIVE_HOST_INSPECTION_ACTION =
  /\b(?:inspect|list|show|check|query|get|read)\b[^.!?;\r\n]{0,160}\b(?:native|host|workstation)\b[^.!?;\r\n]{0,120}\b(?:tmux|terminal\s+sessions?|shell\s+sessions?|process(?:es)?|pids?|systemd|journalctl|ss|lsof|netstat|ports?|listeners?)\b|\b(?:inspect|list|show|check|query|get|read)\b[^.!?;\r\n]{0,160}\b(?:tmux|terminal\s+sessions?|shell\s+sessions?|systemd|journalctl|ss|lsof|netstat)\b|\b(?:tmux|terminal|shell)\s+sessions?\b[^.!?;\r\n]{0,120}\b(?:inspect|list|show|check|query|get|read|running|active)\b|\b(?:inspect|list|show|check|query|get|read)\b[^.!?;\r\n]{0,120}\b(?:running|host|system)\s+process(?:es)?\b/iu;
