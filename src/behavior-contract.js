...
export function formatBehaviorContractForPrompt({ mode = "runtime" } = {}) {
  if (mode === "focused") {
    return [
      ...
      "recover automatically from advice marked autoRecover by choosing the bounded alternative; ask the user only when approval or missing input is genuinely required;",
  ...
  return [
    ...
    "Respect the permission contract: if permissionAdvice is marked autoRecover, continue once with its bounded alternative; otherwise stop and present the exact suggestedCommand/approval path instead of retrying variants or inventing CLI flags.",
    ...
    "For restarted tmux jobs, separate current output from old scrollback with a unique run marker, heartbeat, PID, or durable log timestamp. Do not treat a pane capture as proof of the current state if it only shows pre-restart output.",
  ].join(" ");
}
